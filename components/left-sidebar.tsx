...
const LeftSidebar = () => {
  return (
  <div className="hidden lg:flex relative w-full max-w-[350px] h-auto border-r">
    <div className="w-full max-w-[350px] border-r overflow-hidden overflow-y-auto overscroll-contain perspective-[1px] transform-style-preserve-3d [perspective-origin:right_top] top-[60px] bottom-0 fixed z-[99]">
      <div className="max-w-full mx-auto pl-2">
      <nav className="mb-4">
          ...
          <section>
            ...
            <ul className="space-y-4">
              {followingAccounts.map((account, index) => (
                <Link href={"/"} key={index} className="flex items-center space-x-3">
                  <Image
                    src={account.imgSrc}
                    alt={account.name}
                    width={40}
